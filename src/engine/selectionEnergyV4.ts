...
export function selectionFromEnergy(x:SelectionEnergyInput):SelectionEnergyResult{
 ...
 const minutesCap=x.energy>=75?90:x.energy>=60?82:x.energy>=50?68:x.energy>=30?45:0
 const fatigueRisk=x.energy<30?'critical':x.energy<50?'high':x.energy<62?'medium':'low'
 const available=x.energy>=30
 const canStart=x.energy>=50
 return{available,startChance:canStart?startChance:0,minutesCap,fatigueRisk,reason:!available?'Below 30 energy: unavailable for the matchday squad.':!canStart?'Below 50 energy: bench only; cannot start.':'Available for normal selection.'}
}
...
export function festivalPostMatchEnergy(startEnergy:number,minutes=40,intensity=.78){return postMatchEnergy(startEnergy,minutes,intensity,0)}
export function overnightTournamentRecovery(energy:number){return clamp(Math.round(energy+28),0,100)}
