...
export default function LiveMatchPitch({momentum,homeColor,awayColor,playerIsHome,playerPosition,minute,action='idle',focusPlayer=false,playerName,scoringSide}:{momentum:number;homeColor:string;awayColor:string;playerIsHome:boolean;playerPosition:Position;minute:number;action?:PitchAction;focusPlayer?:boolean;playerName?:string;scoringSide?:'home'|'away'}){
 ...
 const attackHome = scoringSide ? scoringSide === 'home' : momentum >= 0 ? playerIsHome : !playerIsHome
 const actionBall=action==='shot'||action==='goal'?{x:50,y:attackHome?95:5}:action==='cross'?{x:attackHome?72:28,y:attackHome?82:18}:ball
 const labels=['GK','FB','CB','CB','FB','CM','CM','CM','WG','ST','WG']
 ...
 return <div className={'live-pitch relative w-full overflow-hidden rounded-2xl border border-white/10 shadow-2xl'+cameraClass+(focusPlayer?' player-focus':'')} style={{aspectRatio:'3/2'}}>
  ...
  {SHAPE.map((p,i)=><div key={'a'+i} className={'match-dot '+(playerIsHome&&i===userIndex?'user-player':'')} style={{left:`${p.x}%`,top:`${clamp(p.y+shift,5,94)}%`,background:homeColor,animationDelay:`${i*.07}s`}}><span>{labels[i]}</span></div>)}
  {SHAPE.map((p,i)=><div key={'b'+i} className={'match-dot '+(!playerIsHome&&i===userIndex?'user-player':'')} style={{left:`${100-p.x}%`,top:`${clamp(100-p.y-shift,5,94)}%`,background:awayColor,animationDelay:`${i*.07+.2}s`}}><span>{labels[i]}</span></div>)}
  ...
  <div className="pitch-identity"><span className="pitch-you-dot"/>{playerName ?? 'You'} · {playerPosition}<span>{scoringSide ? `${scoringSide === 'home' ? 'Home' : 'Away'} goal` : 'Match overview'}</span></div>
 </div>
}
