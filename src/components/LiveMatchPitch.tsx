...
export default function LiveMatchPitch({momentum,homeColor,awayColor,playerIsHome,minute,action='idle'}:{momentum:number;homeColor:string;awayColor:string;playerIsHome:boolean;minute:number;action?:PitchAction}){
 ...
 return <div className="live-pitch relative w-full overflow-hidden rounded-2xl border border-white/10 shadow-2xl" style={{aspectRatio:'3/2'}}>
  ...
  {SHAPE.map((p,i)=><div key={'a'+i} className="match-dot" style={{left:`${p.x}%`,top:`${clamp(p.y+shift,5,94)}%`,background:homeColor,animationDelay:`${i*.07}s`}}><span>{i+1}</span></div>)}
  {SHAPE.map((p,i)=><div key={'b'+i} className="match-dot" style={{left:`${100-p.x}%`,top:`${clamp(100-p.y-shift,5,94)}%`,background:awayColor,animationDelay:`${i*.07+.2}s`}}><span>{i+1}</span></div>)}
