...
export default function YouthLeagueTab({youthWorld,initialView='fixtures'}:{youthWorld:YouthWorld;initialView?:'fixtures'|'table'}){
 const [view,setView]=useState<'fixtures'|'table'>(initialView)
 ...
 void legacyDivision
 const leaders=Object.values(youthWorld.competitionWorld.statBooks?.[comp.id]?.players??{}).sort((a,b)=>b.goals-a.goals||b.assists-a.assists||((b.apps?b.ratingTotal/b.apps:0)-(a.apps?a.ratingTotal/a.apps:0)))
 ...
 return <div className="flex flex-col gap-2.5">
  <div className="flex gap-1.5 p-1 rounded-xl bg-[#0f0f0d] border border-ks-border">{(['fixtures','table'] as const).map(v=><button key={v} onClick={()=>setView(v)} className={`flex-1 rounded-lg py-2 font-display tracking-widest text-[10px] uppercase ${view===v?'bg-ks-gold text-ks-black':'text-ks-muted'}`}>{v}</button>)}</div>
  ...
  {view==='table'?<><div className="rounded-lg border border-ks-border bg-[#0f0f0d] overflow-hidden"><div className="grid grid-cols-[1.75rem_1fr_2rem_2rem_2rem] gap-1 px-3 py-2 text-[9px] text-ks-muted uppercase"><span>#</span><span>team</span><span>P</span><span>GD</span><span>PTS</span></div>{table.map((s,i)=><div key={s.teamId} className={`grid grid-cols-[1.75rem_1fr_2rem_2rem_2rem] gap-1 px-3 py-2 text-[11px] border-t border-ks-border/30 ${s.teamId===teamId?'bg-ks-gold/10 text-ks-gold':'text-ks-ink'}`}><span>{i+1}</span><span className="truncate">{names.get(s.teamId)}</span><span>{s.played}</span><span>{s.goalDifference>0?'+':''}{s.goalDifference}</span><b>{s.points}</b></div>)}</div><div className="rounded-lg border border-ks-border bg-[#0f0f0d] p-3"><div className="font-display text-[10px] tracking-widest text-ks-gold uppercase mb-2">competition leaders</div>{leaders.length?<div className="space-y-1">{leaders.slice(0,5).map((p,i)=><div key={p.playerId} className="grid grid-cols-[1.2rem_1fr_2rem_2rem] text-[10px]"><span className="text-ks-muted">{i+1}</span><span className="text-ks-ink truncate">{p.name}</span><span className="text-ks-muted">{p.assists}A</span><b className="text-ks-gold text-right">{p.goals}G</b></div>)}</div>:<div className="text-[11px] text-ks-muted">Leaderboards populate from real V5 competition match data once fixtures begin.</div>}</div></>:<div className="rounded-lg border border-ks-border bg-[#0f0f0d] overflow-hidden">{fixtures.map((f:CompetitionFixture)=><div key={f.id} className="px-3 py-3 border-b border-ks-border/30 text-xs flex justify-between gap-2"><span className="text-ks-muted">R{f.round}</span><span className="flex-1 text-ks-ink">{names.get(f.homeTeamId)} <b>{f.played?`${f.homeGoals}–${f.awayGoals}`:'vs'}</b> {names.get(f.awayTeamId)}</span></div>)}</div>}
 </div>
}
